...
def crown_ibp_cifar_cnn_3layer_fixed_kernel_3_width_16(eps):
    ...
    load_model(model, f'models_weights/crown-ibp_models/cifar_crown_{eps}/cifar_8_large/cnn_3layer_fixed_kernel_3_width_16_best.pth')


def ibp_mnist_large(eps):
    assert eps in ['2', '4']

    model = IBP_large(in_ch=3, in_dim=28, linear_size=512)
    load_model(model, f'models_weights/models_crown-ibp_dm-large/mnist_dm_large_0.{eps}/IBP_large_best.pth')
